...
function preserveDetailsState() {
  return [...workspace.querySelectorAll("details[open] > summary")].map((node) => node.textContent || "");
}

function restoreDetailsState(labels) {
  if (!labels?.length) return;
  [...workspace.querySelectorAll("details > summary")].forEach((summary) => {
    if (labels.includes(summary.textContent || "")) summary.parentElement.open = true;
  });
}

function watchRun(jobId) {
  if (runSource && runSourceJob === jobId) return;
  stopRunEvents();
  runSourceJob = jobId;
  runSource = new EventSource(`/api/jobs/${encodeURIComponent(jobId)}/events`);
  runSource.onmessage = (event) => {
    let type = "";
    try { type = JSON.parse(event.data)?.type || ""; } catch {}
    if (refreshTimer) clearTimeout(refreshTimer);
    refreshTimer = setTimeout(() => renderRunDetail(jobId, { silent: true, preserveDetails: true }), 120);
    if (["job.completed", "job.failed", "job.cancelled"].includes(type)) {
      runSource?.close(); runSource = null; runSourceJob = "";
    }
  };
  runSource.onerror = () => {
    // EventSource reconnects automatically; durable state remains in SQLite.
  };
}
...
function runHero(job) {
  ...
  return `<article class="run-hero"><div class="run-hero-grid"><div><div class="run-state ${phaseClass(job)}"><i></i><span>${escapeHtml(phaseText(job))}</span></div><h2>${escapeHtml(model.label_zh || model.label || job.recipe)}</h2><p class="run-sub">${job.total_images} 张图片 · ${escapeHtml(job.gpu)}${parallel > 1 ? ` · 并行 ×${parallel}` : " · 串行"}</p></div><div class="run-stat"><strong>${p.done}<span class="muted">/${p.total}</span></strong><small>已处理</small></div></div><div class="progress-track" aria-label="${p.percent}%"><span style="width:${p.percent}%"></span></div><div class="run-inline-meta"><span><b>${p.percent}%</b> 完成</span><span>${escapeHtml(timeAgo(job.updated_at))}</span><span>${job.queue?.ahead ? `前方 ${job.queue.ahead} 个任务` : ""}</span><span>${job.cost_usd != null ? `估算成本 ${money(job.estimated_cost_usd ?? job.cost_usd)}` : ""}</span></div></article>`;
}

function runRow(job) {
  const model = modelById(job.recipe);
  const p = progress(job);
  return `<a class="run-row" href="#/runs/${encodeURIComponent(job.id)}"><div class="run-row-title"><strong>${escapeHtml(model.label_zh || model.label || job.recipe)}</strong><small>${escapeHtml(job.id)} · ${escapeHtml(timeAgo(job.created_at))}</small></div><div class="run-row-state"><strong>${escapeHtml(phaseText(job))}</strong><span>${escapeHtml(job.gpu)} · ×${Number(job.config?.parallelism || 1)}</span></div><div class="run-row-progress">${p.done} / ${p.total} · ${p.percent}%</div><div class="run-row-cost" title="估算成本">${money(job.estimated_cost_usd ?? job.cost_usd)}</div><div class="chevron">›</div></a>`;
}
...
export async function renderRunDetail(jobId, { silent = false, preserveDetails = false } = {}) {
  const openDetails = preserveDetails ? preserveDetailsState() : [];
  ...
  try {
    ...
    $('[data-cancel]')?.addEventListener("click", async () => {
      try {
        ...
        if (result.cancel_confirmed === false) toast("远程取消未确认；可再次点击停止重试", "bad");
        else toast("远程任务已确认停止");
        await refreshRuntime();
        renderRunDetail(job.id);
    ...
    if (!["completed", "failed", "cancelled"].includes(job.status)) watchRun(job.id); else stopRunEvents();
  } catch (error) {
